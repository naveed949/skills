...
The idea `ask-matt` gives you to think with is the **flow** — a path *through* the skills rather than a single one. Most work runs along one **main flow** (idea → ship: grill → spec → tickets → implement → review, with an optional comprehension quiz before the merge), **on-ramps** merge onto it (a blindspot pass for unfamiliar territory; a triage lane for incoming bugs and requests; a diagnosis lane for something broken), a separate codebase-health lane generates ideas that feed the main flow, and everything else is a **standalone** you reach for on its own. Ask a question and you get placed on the right flow, at the right step — not just handed a tool.
